...
#class TeXLive2008(_path_lists):
#    __doc__ = _('TeXLive 2008')
#    detail = _('Install TeXLive 2008 and enable Chinese support. '
#       'You can create a LaTeX file which contains Chinese characters. '
#       'Then use "texpdf filename.tex" command to build PDF files. '
#       'TeXLive is obtained from http://www.tug.org/texlive/')
#    category = 'latex'
#    time = 819
#    size = 1872912 * 1000
#    logo = 'texlive.png'
#    def __update_etc_environment(self):
#        fn='/etc/environment'
...
#                    f.seek(0,0)
#                    f.writelines(content)
#    def __chinese_font(self):
#        #FIXME Regenerate CJK font for TeXLive 2008, or use zh-spacing
#        f = R(['http://tdt.sjtu.edu.cn/S/Texlive/YueWang-zhfonts-final_1.01.tar.bz2',
#'http://bj.soulinfo.com/~hugang/tex/tex2007/YueWang-zhfonts-final_1.01.tar.bz2'],
#288090304, 'a549cfa737344d17135f008ed257f9ef1931a54c').download()
#        FileServer.chdir_local()
#        try:
#            run("bunzip2 -fv %s" %f)
#            f = f[:-4]
#            run("tar xf %s"%f)
#            run("rm -rf ~/.texlive2008")
#            run("mv .texlive2007 ~/.texlive2008")
#            if get_arch()==64:
#                path = "/usr/local/texlive/2008/bin/x86_64-linux/"
#            else:
#                path = "/usr/local/texlive/2008/bin/i386-linux/"
#            run("%sfmtutil --all"%path, ignore_error = True)
#        finally:
#            FileServer.chdir_back()
#    def __compile_script(self):
#        import os
#        if not os.path.exists('../data/files/texpdf'):
#            raise Exception('Current dir: '+os.getcwd(),
#                            self.debug_path1, 
#                            self.debug_path2,
#                            self.debug_path3)
#        gksudo("cp %s /usr/local/bin/"%"../data/files/texpdf")
#        gksudo("chmod a+x /usr/local/bin/texpdf")
#    def __install_main_program(self):
#        FileServer.chdir_local()
#        try:
#            iso = R(['http://tdt.sjtu.edu.cn/S/Texlive/texlive2008-20080822.iso',],
#                2617149440, 'abcc6939206a1b365c336943748390ab1a98d42f'
#                ).download()
#            import tempfile
#            temp = tempfile.NamedTemporaryFile(mode='w')
#            temp.write("I")
#            temp.flush()
#            gksudo("mkdir /mnt/texlive", ignore_error = True)
#            gksudo("mount -t iso9660 -o iocharset=utf8,loop %s /mnt/texlive"%iso)
...
#            gksudo("rmdir /mnt/texlive")
#            temp.close()
#        finally:
#            FileServer.chdir_back()
#    def install(self):
#        import os
#        self.__install_main_program();  self.debug_path1 = os.getcwd()
#        self.__update_etc_environment(); self.debug_path2 = os.getcwd()
#        self.__chinese_font(); self.debug_path3 = os.getcwd()
#        self.__compile_script()
#        notify(_('TeXLive is installed.'), _('TeXLive will not work until you restart the computer.'))
#    def __init__(self):
#        import os
#        self.paths = [
#'/usr/local/texlive/2008/',
#os.path.expanduser('~/.texlive2008'),
#'/usr/local/bin/texpdf',
#                      ]
