...
def __gtk():
     import gtk
     try: return [row(_('Gtk version:'), '.'.join(map(str, gtk.gtk_version)), D+'umut_icons/tar.png')]
     except: traceback.print_exc(file=sys.stderr)
     return []
 
def __pygtk():
     import gtk
     try: return [row(_('Pygtk version:'), '.'.join(map(str, gtk.pygtk_version)), D+'umut_icons/tar.png' )]
     except: traceback.print_exc(file = sys.stderr)
